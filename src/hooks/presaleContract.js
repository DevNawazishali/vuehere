...
import { presaleBuy, presaleRelease } from '@/setup/address';
...
const presaleContract = () => {
  const contract = web3Contract();
  return new contract.eth.Contract(presale, presaleBuy);
};

const releaseContract = () => {
  const contract = web3Contract();
  return new contract.eth.Contract(presale, presaleRelease);
};
...
export const buyMBT = async (amount, activeSymbol, code) => {
  ...
  const isApprove_ = await isApproveContract(activeSymbol.token, presaleBuy);
  if (!isApprove_) {
    const is = await Approve(activeSymbol.token, presaleBuy);
  }
