...
export function encrypt(key, iv, data) { // AES加密（node<==java,c,o-c;iv-向量)
  const cipher = crypto.createCipheriv('aes-128-cbc', key, iv);
  const crypt = cipher.update(data, 'utf8', 'binary') + cipher.final('binary');
  // eslint-disable-next-line no-buffer-constructor
  return new Buffer(crypt, 'binary').toString('base64');
}

export function decrypt(key, iv, crypt) { // AES解密（node<==java,c,o-c;iv-向量)
  // eslint-disable-next-line no-buffer-constructor
  const newCrypt = new Buffer(crypt, 'base64').toString('binary');
  const decipher = crypto.createDecipheriv('aes-128-cbc', key, iv);
  return decipher.update(newCrypt, 'binary', 'utf8') + decipher.final('utf8');
}
